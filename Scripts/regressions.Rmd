...
\begin{aligned}
  \textrm{Décès}_{ijt} = \beta_0 &+ \beta_1 \textrm{Année}_i + \beta_2 \textrm{Période}_t + \beta_3 (\textrm{Année}_i \times \textrm{Période}_t)\\
   &+ \beta_4 \textrm{Week-end} + \alpha_j + \epsilon_i
\end{aligned}
\end{equation}
...
reg1820_conf1_weekend <- felm(ndec ~ dicho_2019 + dicho_2020 + 
...
reg1420_conf1_weekend <- felm(ndec ~ dicho_2015 + dicho_2016 + dicho_2017 + 
                                dicho_2018 + dicho_2019 + dicho_2020 + 
                                periode23 + dicho_2020:periode23 + weekend,
                              data = deces_jours[periode %in% 1:3 & 
                                                   ADEC %in% 2014:2020])
...
tab <- modelsummary(list(reg1820_conf1, 
                         reg1820_conf1_weekend, 
                         reg1820_conf1_fe,
                         reg1420_conf1, 
                         reg1420_conf1_weekend, 
                         reg1420_conf1_fe),
                    output = "data.frame",
...
tab %>% 
  ...
  add_header_above(c("", "2018-2019" = 3, "2014-2019" = 3),
                   bold = T) %>%
...
reg1820_conf2 <- felm(ndec ~ dicho_2019 + dicho_2020 + 
                      ...
                      data = deces_jours[periode %in% c(1,4) & 
                                           ADEC %in% 2018:2020])
reg1820_conf2_weekend <- felm(ndec ~ dicho_2019 + dicho_2020 + 
                                periode4 + dicho_2020:periode4 +
                                weekend,
                              data = deces_jours[periode %in% c(1,4) & 
                                                   ADEC %in% 2018:2020])
...
reg1420_conf2 <- felm(ndec ~ dicho_2015 + dicho_2016 + dicho_2017 + 
                      ...
                      data = deces_jours[periode %in% c(1,4) & 
                                           ADEC %in% 2014:2020])
reg1420_conf2_weekend <- felm(ndec ~ dicho_2015 + dicho_2016 + dicho_2017 + 
                                dicho_2018 + dicho_2019 + dicho_2020 + 
                                periode4 + dicho_2020:periode4 +
                                weekend,
                              data = deces_jours[periode %in% c(1,4) & 
...
tab <- modelsummary(list(reg1820_conf2,
                         reg1820_conf2_weekend,
                         reg1420_conf2,
                         reg1420_conf2_weekend),
                    output = "data.frame",
                    #vcov = sandwich::vcovHC,
                    estimate = "{estimate}{stars}",
                    statistic = "({conf.low} ; {conf.high})",
...
tab %>% 
  ...
  #column_spec(column = 2, bold = T) %>%
  row_spec(9:10, bold=TRUE) %>%
  add_header_above(c("", "2018-2019" = 2, "2014-2019" = 2),
                   bold = T) %>%
  footnote(general = c("Source : INSEE, 2014 à 2020.",
...
reg1820_per23_weekend <- felm(ndec ~ dicho_2019 + dicho_2020 + 
                                periode23 + dicho_2020:periode23 + weekend,
                      data = deces_jours[periode %in% 1:3 & 
                                           ADEC %in% 2018:2020])
...
reg1420_per23_weekend <- felm(ndec ~ dicho_2015 + dicho_2016 + dicho_2017 + 
                                dicho_2018 + dicho_2019 + dicho_2020 + 
                                periode23 + dicho_2020:periode23 + weekend,
                              data = deces_jours[periode %in% 1:3 & 
                                                   ADEC %in% 2014:2020])
...
tab <- modelsummary(list(reg1820_per23, 
                         reg1820_per23_weekend, 
                         reg1820_per23_fe,
                         reg1420_per23, 
                         reg1420_per23_weekend, 
                         reg1420_per23_fe),
                    output = "data.frame",
                    #vcov = sandwich::vcovHC,
                    estimate = "{estimate}{stars}",
...
tab %>% 
  kbl(longtable = F, booktabs = T,
      escape = F, format = "latex",
      caption = "Effet du premier confinement et de la période d'entre-deux-confinement sur le nombre de décès journaliers des 18-29 ans", 
      linesep = "",
      align = c("l", rep("c", 6))) %>% 
  kable_styling(position ="center", 
                latex_options = c("HOLD_position")) %>%
...
regfem1820_conf1_fe <- felm(ndec_fem ~ dicho_2019 + dicho_2020 + periode2 + 
                              dicho_2020:periode2 + weekend|numjour|0|0,
...
tab %>% 
  ...
  #column_spec(column = 2, bold = T) %>%
  row_spec(9:10, bold=TRUE) %>%
  add_header_above(c("", rep(c("2018-2019" = 1, 
                               "2014-2019" = 1),2)),
                   bold = F, line = F) %>%
    add_header_above(c("", "Femmes" = 2, "Hommes" = 2),
                   bold = T) %>%
  footnote(general = c("Source : INSEE, 2014 à 2020.",
                       "Champ : décès journaliers de personnes agées de 18 à 29 ans survenus en France métropolitaine entre 2014 et 2020, sur la période du 1er janvier au 10 mai.",
                       "Seuils de significativité : p < 0,1 *; p < 0,05 **, p < 0,01 ***."), 
...
lrtest(reg1820_conf1, reg1820_conf1_weekend)
lrtest(reg1820_conf1_weekend, reg1820_conf1_fe)
