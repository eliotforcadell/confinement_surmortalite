...
<<label = package_data ,fig=FALSE, echo=FALSE, include=FALSE, cache = T, results = hide>>=
pacman::p_load(data.table, magrittr, ggplot2, ggpubr, kableExtra, questionr, lubridate)
...
dc_jour <-  freq(deces$date_dec) 
dc_jour$jour <- rownames(dc_jour)
dc_jour$jour <- ymd(dc_jour$jour)
str(dc_jour$jour)
summary(dc_jour$n)
sd(dc_jour$n)
summary(dc_jour$n[year(dc_jour$jour) == 2020])
sd(dc_jour$n[year(dc_jour$jour) == 2020])
sd(dc_jour$n[year(dc_jour$jour) == 2018])
sd(dc_jour$n[year(dc_jour$jour) == 2019])
summary(dc_jour$n[year(dc_jour$jour) == 2018])
summary(dc_jour$n[year(dc_jour$jour) == 2019])
@

\begin{center}
\begin{figure}[h!]
\caption{\label{Graph_descr}Tendances mortalité par jour (gaussian smoothing)}
<<echo=F,eval=T,fig = T,width = 8,height = 4>>=

ggplot() + 
    geom_density(data = deces, aes(x=date_dc_graph, group = ADEC, color = as.factor(ADEC)), 
                 kernel = "gaussian", bw = 7)   +
